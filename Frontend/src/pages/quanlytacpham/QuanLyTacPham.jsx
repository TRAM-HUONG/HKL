...
const QuanLyTacPham = () => {
        ...
        if (window.confirm(confirmMsg)) {
            try {
                const endpoint = type === 'truyen' 
                    ? `http://localhost:5173//api/author/delete-truyen/${id}`
                    : `http://localhost:5173//api/author/delete-ban-thao/${id}`;
    ...
    const handleHoanThanh = async (id) => {
        if (window.confirm("Xác nhận tác phẩm này đã kết thúc và chuyển sang trạng thái 'Hoàn thành'?")) {
            try {
                const res = await axios.put(`http://localhost:5173//api/author/hoan-thanh-truyen/${id}`);
                if (res.data.success) {
    ...
    const fetchLibrary = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`http://localhost:5173//api/author/my-library/${matk}`);
            if (res.data.success) {
                        ...
                        {stories.length > 0 ? stories.map((s) => (
                            <tr key={s.ma_so}>
                                <td data-label="Ảnh/Mã" className="col-image">
                                    {type === 'truyen' ? (
                                        <img 
                                            src={`http://localhost:5173//images/${s.hinh_anh}`} 
                                            alt="cover" 
