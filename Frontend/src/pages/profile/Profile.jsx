...
const Profile = () => {
  ...
  const fetchDoanhThu = async (matg) => {
    if (!matg) return;
    try {
      const response = await fetch(`https://hkl-backend.onrender.com/api/user/tac-gia/doanh-thu/${matg}`);
      if (response.ok) {
  ...
  const handleWithdrawRequest = async () => {
    ...
    try {
      const res = await fetch(`https://hkl-backend.onrender.com/api/user/withdraw/request`, {
        method: 'POST',
  ...
  const fetchLichSu = async (madg) => {
    if (!madg) return;
    try {
      const response = await fetch(`https://hkl-backend.onrender.com/api/lich-su/${madg}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchFinance = async (matk) => {
    if (!matk) return;
    try {
      const response = await fetch(`https://hkl-backend.onrender.com/api/user/financial-info/${matk}`);
      if (response.ok) {
        const data = await response.json();
        setFinance(data);
      }
  ...
  const handleUpdateInfo = async (e) => {
    ...
    try {
      const response = await fetch(`https://hkl-backend.onrender.com/api/user/update`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteLichSu = async (mat) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa truyện này khỏi lịch sử đọc?")) {
      try {
        const response = await fetch(`https://hkl-backend.onrender.com/api/lich-su/delete`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ 
              ...
              {lichSu.length > 0 ? (
                lichSu.map((item, index) => (
                  <div key={index} className="history-item">
                    <img 
                      src={`https://hkl-backend.onrender.com/images/${item.hinhanh}`} 
                      alt={item.tent} 
