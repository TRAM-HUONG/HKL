...
const Profile = () => {
  ...
  const fetchDoanhThu = async (matg) => {
    if (!matg) return;
    try {
      const response = await fetch(`http://localhost:5173//api/user/tac-gia/doanh-thu/${matg}`);
      if (response.ok) {
  ...
  const handleWithdrawRequest = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5173//api/user/withdraw/request`, {
        method: 'POST',
  ...
  const fetchLichSu = async (madg) => {
    if (!madg) return;
    try {
      const response = await fetch(`http://localhost:5173//api/lich-su/${madg}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchFinance = async (matk) => {
    if (!matk) return;
    try {
      const response = await fetch(`http://localhost:5173//api/user/financial-info/${matk}`);
      if (response.ok) {
        const data = await response.json();
        setFinance(data);
      }
  ...
  const handleDeleteLichSu = async (mat) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa truyện này khỏi lịch sử đọc?")) {
      try {
        const response = await fetch(`http://localhost:5173//api/lich-su/delete`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ 
