...
const ChiTietBanthao = () => {
    ...
    const fetchDetail = async () => {
        try {
            const res = await axios.get(`https://hkl-backend.onrender.com/api/admin/ban-thao/detail/${mabt}`);
            if (res.data.success) {
    ...
    const handleApprove = async () => {
        if (window.confirm("Bạn có chắc chắn muốn duyệt chương này lên trang chủ?")) {
            try {
                const res = await axios.put(`https://hkl-backend.onrender.com/api/admin/ban-thao/approve/${mabt}`);
                if (res.data.success) {
