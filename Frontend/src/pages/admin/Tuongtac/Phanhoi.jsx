...
const Phanhoi = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await axios.get('https://hkl-backend.onrender.com/api/admin/phan-hoi/all');
            if (res.data.success) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Bạn có chắc muốn xóa phản hồi này?")) {
            try {
                const res = await axios.delete(`https://hkl-backend.onrender.com/api/admin/phan-hoi/delete/${id}`);
                if (res.data.success) {
