...
const Danhgia = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await axios.get('https://hkl-backend.onrender.com/api/admin/danh-gia/all');
            if (res.data.success) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Bạn có chắc chắn muốn xóa đánh giá này không?")) {
            try {
                const res = await axios.delete(`https://hkl-backend.onrender.com/api/admin/danh-gia/delete/${id}`);
                if (res.data.success) {
