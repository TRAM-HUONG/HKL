...
const ViewApprove = () => {
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            try {
                ...
                const res = await axios.get(`http://localhost:5173//api/admin/truyen-detail/${id}`);
                if (res.data.success) {
    ...
    const handleApprove = async () => {
        if (window.confirm("Xác nhận phê duyệt truyện này lên hệ thống?")) {
            try {
                const res = await axios.put(`http://localhost:5173//api/admin/approve-truyen/${id}`);
                if (res.data.success) {
        ...
        <div className={styles['admin-glass-content']}>
            ...
            <div className={styles['admin-detail-wrapper']}>
                ...
                <div className={styles['detail-poster']}>
                    <img 
                        src={`http://localhost:5173//images/${data.hinh_anh}`} 
                        alt="cover" 
