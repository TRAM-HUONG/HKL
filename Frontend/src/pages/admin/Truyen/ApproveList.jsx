...
const ApproveList = () => {
    ...
    const fetchAllStories = async () => {
        setLoading(true);
        try {
            const res = await axios.get('http://localhost:5173//api/admin/truyen-manage/all');
            if (res.data.success) {
    ...
    const handleReject = async (id) => {
        if (window.confirm("Bạn có chắc chắn muốn TỪ CHỐI duyệt truyện này?")) {
            try {
                await axios.put(`http://localhost:5173//api/admin/truyen/reject/${id}`);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Bạn có chắc chắn muốn GỠ BỎ truyện này vĩnh viễn không?")) {
            try {
                await axios.delete(`http://localhost:5173//api/admin/truyen/delete/${id}`);
                    ...
                    <tbody>
                        {currentData.length > 0 ? currentData.map(s => (
                            <tr key={s.mat}>
                                ...
                                <td>
                                    <img 
                                        src={`http://localhost:5173//images/${s.hinhanh}`} 
