...
const ApproveList = () => {
    ...
    const fetchAllStories = async () => {
        setLoading(true);
        try {
            const res = await axios.get('https://hkl-backend.onrender.com/api/admin/truyen-manage/all');
            if (res.data.success) {
    ...
    const handleReject = async (id) => {
        if (window.confirm("Bạn có chắc chắn muốn TỪ CHỐI duyệt truyện này?")) {
            try {
                await axios.put(`https://hkl-backend.onrender.com/api/admin/truyen/reject/${id}`);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Bạn có chắc chắn muốn GỠ BỎ truyện này vĩnh viễn không?")) {
            try {
                await axios.delete(`https://hkl-backend.onrender.com/api/admin/truyen/delete/${id}`);
                    ...
                    <tbody>
                        {currentData.length > 0 ? currentData.map(s => (
                            <tr key={s.mat}>
                                ...
                                <td>
                                    <img 
                                        src={`https://hkl-backend.onrender.com/images/${s.hinhanh}`} 
