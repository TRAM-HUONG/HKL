...
const ChiTietTruyen = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5173//api/truyen/${mat}`)
      .then((res) => res.json())
      .then((data) => {
        setTruyen(data);
        setLoading(false);
  ...
  const loadDanhGia = () => {
    fetch(`http://localhost:5173//api/truyen/${mat}/danh-gia`)
      .then((res) => res.json())
  ...
  const handleChapterClick = async (chuong) => {
    ...
    try {
      ...
      const checkRes = await fetch(`http://localhost:5173//api/chuong/check-quyen?mabt=${chuong.mabt}&madg=${madg}`);
  ...
  const thucHienMuaChuongLe = async (chuong) => {
    try {
      const res = await fetch("http://localhost:5173//api/chuong/mua-le", {
        method: "POST",
  ...
  const handleBuyFullStory = async () => {
    ...
    if (window.confirm(`Xác nhận dùng ${giaFull} Xu để mua trọn bộ truyện này?`)) {
      try {
        const res = await fetch("http://localhost:5173//api/truyen/mua-tron-goi", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            madg: user.MADG || user.madg,
  ...
  const handleSendReview = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5173//api/truyen/danh-gia`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          mat: mat,
  ...
  const handleSendPhanHoi = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5173//api/binh-luan`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleDeletePH_DG = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5173//api/binh-luan/phan-hoi/${id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteDanhGia = async (madgia) => {
    ...
    try {
      const response = await fetch(`http://localhost:5173//api/truyen/danh-gia/${madgia}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  const handleStartReading = () => {
    ...
    setShowModal(true);
    fetch(`http://localhost:5173//api/chuong/truyen/${mat}`)
      .then((res) => res.json())
      .then((data) => {
        ...
        <div className="chitiet-content">
          <div className="chitiet-left">
            <img
              ...
              src={`http://localhost:5173//images/${truyen.hinhanh || truyen.HINHANH || ""}`}
