...
const ChiTietTruyen = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://hkl-backend.onrender.com/api/truyen/${mat}`)
      .then((res) => res.json())
      .then((data) => {
        setTruyen(data);
        setLoading(false);
  ...
  const loadDanhGia = () => {
    fetch(`https://hkl-backend.onrender.com/api/truyen/${mat}/danh-gia`)
      .then((res) => res.json())
 ...
 const handleChapterClick = async (chuong) => {
    ...
    try {
      ...
      const checkRes = await fetch(`https://hkl-backend.onrender.com/api/chuong/check-quyen?mabt=${chuong.mabt}&madg=${madg}`);
  ...
  const handleBuyFullStory = async () => {
    ...
    if (window.confirm(`Xác nhận dùng ${giaFull} Xu để mua trọn bộ truyện này?`)) {
      try {
        const res = await fetch("https://hkl-backend.onrender.com/api/truyen/mua-tron-goi", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            madg: user.MADG || user.madg,
  ...
  const handleSendReview = async () => {
    ...
    try {
      const response = await fetch(`https://hkl-backend.onrender.com/api/truyen/danh-gia`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          mat: mat,
  ...
  const handleSendPhanHoi = async () => {
    ...
    try {
      const response = await fetch(`https://hkl-backend.onrender.com/api/binh-luan`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleDeleteDanhGia = async (madgia) => {
    ...
    try {
      const response = await fetch(`https://hkl-backend.onrender.com/api/truyen/danh-gia/${madgia}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  const handleStartReading = () => {
    ...
    setShowModal(true);
    fetch(`https://hkl-backend.onrender.com/api/chuong/truyen/${mat}`)
      .then((res) => res.json())
      .then((data) => {
        ...
        <div className="chitiet-content">
          <div className="chitiet-left">
            <img
              ...
              src={`https://hkl-backend.onrender.com/images/${truyen.hinhanh || truyen.HINHANH || ""}`}
          ...
          <div className="chitiet-info">
            ...
            <div className="chitiet-meta">
                <p>✍️ <strong>Tác giả:</strong> {truyen.ten_tac_gia || truyen.TENTG || "Đang cập nhật"}</p> 
                  ...
                  {danhSachChuong.map((chuong, index) => (
                    ...
                    >
                      <div>
                        ...
                        <span className="chuong-name">{chuong.tenbt || chuong.TENBT}</span>
                      </div>
                      <span className="chuong-price" style={{ fontSize: '0.9rem', color: '#ffcc00', fontWeight: 'bold' }}>
                        {(chuong.gia_xu || chuong.GIA_XU) > 0 ? `${chuong.gia_xu || chuong.GIA_XU} Xu` : "Miễn phí"}
                      </span>
                    </div>
                  ))}
                </div>
