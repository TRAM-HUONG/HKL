...
const DocTruyen = () => {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      setLoading(true);
      try {
        const res = await fetch(`http://localhost:5173//api/chuong/noidung/${mabt}`);
        ...
        if (giaChuong > 0) {
          ...
          if (!madg) {
            ...
            const checkRes = await fetch(`http://localhost:5173//api/chuong/check-quyen?mabt=${mabt}&madg=${madg}`);
  ...
  const loadBinhLuan = () => {
    fetch(`http://localhost:5173//api/binh-luan/chuong/${mabt}`)
  ...
  const handleSendBinhLuan = async () => {
    ...
    try {
        const res = await fetch(`http://localhost:5173//api/binh-luan`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteBL = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5173//api/binh-luan/${id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const saveReadingHistory = (madg, mat, tenbt) => {
    fetch(`http://localhost:5173//api/lich-su/update`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const fetchDanhSachChuong = (maTruyen) => {
    fetch(`http://localhost:5173//api/chuong/truyen/${maTruyen}`)
      .then((res) => res.json())
      .then((data) => {
